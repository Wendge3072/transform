...
uint32_t Bitset::count(uint32_t from, uint32_t to) noexcept {
  uint32_t count = 0;

  // count 1s in the range [from, to]
  uint32_t fromByte = from / 8;
  uint32_t toByte = to / 8;
  if(fromByte == toByte) {
    uint8_t mask = 0xFF >> (8 - (to - from + 1));
    count = popcount(data[fromByte] & (mask << (from % 8)));
  } else {
    uint8_t mask = 0xFF >> (from % 8);
    count = popcount(data[fromByte] & mask);
    for(uint32_t i = fromByte + 1; i < toByte; i++) {
      count += popcount(data[i]);
    }
    mask = 0xFF >> (8 - (to % 8 + 1));
    count += popcount(data[toByte] & mask);
  }
  return count;
}
