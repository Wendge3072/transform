...
uint32_t Block::getValidPageCount(uint32_t slpnOffset, uint32_t elpnOffset) {
  uint32_t ret = 0;

  if (ioUnitInPage == 1) {
    ret = pValidBits->count(slpnOffset, elpnOffset);
  }
  else {
    for (auto &iter : validBits) {
      if (iter.any()) {
...
void Block::printValidBits() {
  if (ioUnitInPage == 1) {
    for(uint32_t idx = 0; idx < pageCount; idx++) {
      if(pValidBits->test(idx)) {
        debugprint(LOG_FTL, "Valid in page: %u", idx);
      }
    }
  }
}
