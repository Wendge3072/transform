...
FTL::FTL(ConfigReader &c, DRAM::AbstractDRAM *d) : conf(c), pDRAM(d) {
  ...
  switch (conf.readInt(CONFIG_FTL, FTL_MAPPING_MODE)) {
    case PAGE_MAPPING:
      ...
      break;
    case HYBRID_MAPPING:
      pFTL = new HybridMapping(conf, param, pPAL, pDRAM);
      break;
  }
  ...
  // debugprint(LOG_FTL, "Page max performance %u", param.pageCountToMaxPerf);
  // debugprint(LOG_FTL, "IO unit in page %u", param.ioUnitInPage);
